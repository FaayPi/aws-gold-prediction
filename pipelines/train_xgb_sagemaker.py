"""
SageMaker-kompatibles Training für XGBoost (GC=F).
Liest Trainingsdaten aus dem Channel "training", nutzt 3y Rolling Window,
Feature-Engineering (optimiert) und speichert Modell + Scaler + Feature-Liste.
"""
...
try:
    ...
    raise SystemExit("xgboost nicht installiert. Bitte `pip install xgboost` ausführen.") from e
...
def main(args=None):
    ...
    # Hyperparameters (können bei Bedarf angepasst werden oder via Grid Search kommen)
    params_base = {
        "n_estimators": 800,
        "max_depth": 3,
        "learning_rate": 0.05,
    ...
    # Save artifacts
    joblib.dump(model_day, model_dir / "model_day.joblib")
    joblib.dump(model_week, model_dir / "model_week.joblib")
    joblib.dump(scaler_day, model_dir / "scaler_day.joblib")
    joblib.dump(scaler_week, model_dir / "scaler_week.joblib")
    joblib.dump(feats_day, model_dir / "features_day.joblib")
    joblib.dump(feats_week, model_dir / "features_week.joblib")
    print(f"✅ Models saved to {model_dir}")


def model_fn(model_dir):
    # Load artifacts for inference
    model_day = joblib.load(Path(model_dir) / "model_day.joblib")
    model_week = joblib.load(Path(model_dir) / "model_week.joblib")
    scaler_day = joblib.load(Path(model_dir) / "scaler_day.joblib")
    scaler_week = joblib.load(Path(model_dir) / "scaler_week.joblib")
    feats_day = joblib.load(Path(model_dir) / "features_day.joblib")
    feats_week = joblib.load(Path(model_dir) / "features_week.joblib")
    return {
        "model_day": model_day,
